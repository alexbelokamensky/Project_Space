...
from core.config import WINDOW_WIDTH, WINDOW_HEIGHT, IMAGE_PATH, SOUND_PATH, THEMES_PATH
from interface.screens.base_screen import Screen

class MainMenuScreen(Screen):
    def __init__(self, manager):
        self.manager = manager
        self.ui_manager = pgui.UIManager((WINDOW_WIDTH, WINDOW_HEIGHT), os.path.join(THEMES_PATH, "menu_theme.json"))
        self.start_survival_mode_game = pgui.elements.UIButton(relative_rect=pg.Rect(WINDOW_WIDTH/2-100, WINDOW_HEIGHT/2-80, 200, 80),
                                                               text="Survival Mode", manager=self.ui_manager)

        self.exit_game = pgui.elements.UIButton(relative_rect=pg.Rect(WINDOW_WIDTH/2-100, WINDOW_HEIGHT/2+30, 200, 80),
